...
from .multihead_diffatention import MultiheadDiffAttention, RMSNorm, SwiGLU
...
class TransformerEncoder(nn.Module):
    ...
    def __init__(self, num_layers, d_model, nhead, dim_feedforward, dropout, 
                 activation, normalize_before, norm=None):
        
        super().__init__()
        
        self.layers = nn.ModuleList([
            TransformerEncoderLayer(d_model, nhead, i, dim_feedforward, dropout, activation, 
...
class TransformerDecoder(nn.Module):
    ...
    def __init__(self, num_layers, d_model, nhead, dim_feedforward, dropout, activation, 
                 normalize_before, norm=None, return_intermediate=False):
        super().__init__()
        self.layers = nn.ModuleList([
            TransformerDecoderLayer(d_model, nhead, i, dim_feedforward, dropout, activation, 
                                    normalize_before) for i in range(num_layers)])
        self.num_layers = num_layers
        self.norm = norm
...
class TransformerEncoderLayer(nn.Module):
    ...
    def __init__(self, d_model, nhead, depth, dim_feedforward=2048, dropout=0.1,
                 activation="relu", normalize_before=False):
        super().__init__()
        self.self_attn = MultiheadDiffAttention(d_model, nhead, depth, dropout=dropout)
...
class TransformerDecoderLayer(nn.Module):
    ...
    def __init__(self, d_model, nhead, depth, dim_feedforward=2048, dropout=0.1,
        ...
        super().__init__()
        self.self_attn = MultiheadDiffAttention(d_model, nhead, depth, dropout=dropout)
        self.multihead_attn = MultiheadDiffAttention(d_model, nhead, depth, dropout=dropout)
        if activation == "swiglu":
            self.linear1 = nn.Linear(d_model, 2 * dim_feedforward)
            self.activation = SwiGLU()
        else:
    ...
    def forward_post(self, tgt, memory,
        ...
        tgt2 = self.self_attn(q, k, value=tgt, attn_mask=tgt_mask,
                              ...
                              pos=query_pos, query_pos = query_pos)[0]
